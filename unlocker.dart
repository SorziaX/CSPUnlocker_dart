...
Future<int> unlock() async {
  ...
  do {
    ...
    if (srcFileAccess.positionSync() == 0) {
      isFirst = true;
      bufferBytes =
          new Uint8List.fromList(srcFileAccess.readSync(lengthOfRead));
    } else {
    ...
    if (matched) {
      ...
      newFileAccess.writeFromSync(
          bufferBytes, bufferi + bytesLength, bufferBytes.length);
      ...
      newFileAccess.writeFromSync(
          bufferBytes,
          0,
          (isLast ? bufferBytes.length : lengthOfRead) -
              (isFirst ? bytesLength : 0));
    }

    page++;
    if (page % 1000 == 0) printToCMD("过程: ${page / 1000} * 1000KB");
